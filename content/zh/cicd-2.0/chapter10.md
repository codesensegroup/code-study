...
本章主要討論軟體進入生產環境之前的自動化測試管理，包含：

- 自動化測試的自身定位：包含哪些測試、哪些可以被自動化…
- 傳統自動化測試的困境：以前的人做自動化測試遇到哪些困難
- 利於反饋的自動化測試實施策略
- 如何編寫易維護的自動化測試案例
...
1. 「問題認知」：對業務問題本身的理解與認識
2. 「分析」：測試分析與設計 (以最低的測試成本，達到能驗證程式是否解決問題)
3. 「執行」：執行測試，得到測試結果數據
...
- 四個面向
    ...
    - 評判項目：目標是為了找出產品是否有缺限
- 四個象限
    - 第一象限：(手工) 用戶演示demo、可用性測試、探索性測試
    - 第二象限：**(自動化)** 功能驗收測試 => 以用戶的角度
    - 第三象限：**(自動化)** 系統集成測試、組件測試、單元測試 => 技術實現的驗證
    - 第四象限：(自動化/手工) 非功能驗收測試(性能測試、容量測試、可靠性測試…)
- 本章主要討論第二、第三象限的自動化測試
...
#### 「節省時間和執行成本」
  - 測試活動需要經常性重覆以確保軟體品質
  - 在長生命週期、發布頻率高的產品中，時間成本的節省相當明顯
#### 「提升測試覆蓋度」:
  - 相比手動測試，自動化測試可以增加測試的深度和範圍，以提高軟件質量
  - ex: 內存使用、內部程序狀態…
#### 「做手工無法完成的測試」:
...
#### 手動測試與自動化測試的不同
- 自動化測試
  - "無法觀察執行過程"，無主觀意識，它只做腳本要求它做的事
  - 無主動觀察、主動認知和分析能力，沒有創造力
...
- 自動化測試常作為提測前的檢驗標準 (提交給手工測試之前，先運行自動化測試案例)
...
1. 測試分析者分析測試案例，並文檔化
2. 測試執行者執行測試案例，並報告BUG
3. 開發人員修復bug
4. 測試執行者再次執行測試，直至驗證通過
5. 測試自動化專家從測試案例文檔中選出一些重要且變動可能性較小的測試案例
6. 對挑選出來的測試案例編寫自動化腳本，並歸入自動化回歸測試庫

### 10.2.1 傳統自動化測試的特點

#### 1. 測試案例執行成本過高
- 多為黑盒自動化測試案例
- 摸擬真實用戶的介面操作來驅動的系統集成測試
- 處理流程較長、要準備的測試案例較多，需花費較多時間、精力
#### 2. 自動化測試執行頻率低
- 通常是在軟件開發完後(提測階段)、或系統回歸測試(重覆以前全部或部份的測試工作)用
#### 3. 質量反饋滯後
- 大部份的測試案例是回歸測試案例，無法對當前正在開發的新功能進行測試
- 為了能安全執行，經常會通過 sleep 這樣的指令讓流程暫停，再繼續下一流程，因此很耗時
#### 4. 測試環境準備成本高
- 完善的測試數據集
- 整套的運行環境
...
- 大型團隊中，若界面需求的改動若沒有即時通知測試團隊，也會造成測試失敗
- 團隊可能會更傾向忽略這些自動化測試案例的存在
#### 6. 人員依賴性強
- 編寫自動化測試案例很大程度依賴少數測試開發專職人員
- 適合用于發佈版本週期較長，使用傳統瀑布開發方法的團隊
- 軟件版本迭代速度越來越快，這種自動化測試案例的投資回報率越來越低
...
#### 「快、捷、時、信」為持續整合實踐對自動化測試建設的4個基本衡量維度

<alert>
傳統自動化測試存在一些問題，要如何改善，可以從下列四個維度去思考
良好的自動化測試案例應該符合下列四個維度
</alert>
...
| 及時 | 一但功能發生變化，就能馬上告知本次代碼變更對軟體質量的影響 <br>(但若沒有新增此次改動對應的測試案例，會導至反饋速度的降低)                          |
...
- 隨著技術發展，軟體架構向「服務化」和「微服務化」方向過渡
  - => 掀起大規模分散式應用服務浪潮
- 軟體服務由「單體應用架構」拆分成「服務導向的架構(SOA)」
  - 服務之間透過RPC等方式進行溝通
...
- 自動化測試不是一新鮮事物，每種開發語言都有對應的測試框架，常常不只一種
...
- 多數自動化測試案例是由開發工程師自行負責編寫
  - 若同一功能由一人編寫測試，一人編寫代碼，就會產生溝通成本
    - (傳統自動化測試的缺點)

#### 3. 從測試金字塔的中間層向上下兩端擴展
- ex: 採用微服務架構的服務端軟體 => 從契約層開始著手
- ex: 手機端APP => 從元件級或API級的測試開始入手

#### 4. 自動化測試案例的質量比數量重要
- 在能達到品質目的的前提下，自動化測試的案例越少越好 (降低成本)
...
- 自動化測試應盡可能與生產程式碼同步變化
  - 沒有同步變化，執行就會失敗
  - 當開發人員執行自動化測試失敗後，就應該馬上進行修改
    - (可能是功能出錯，也可能是測試需要修改)
- 破窗效應：
  - 犯罪學理論：環境中的不良現象如果被放任存在，會誘使人們仿效，甚至變本加厲
  - 越來越多測試失敗卻沒有受到關注，久了就對自動化測試的結果視而不見

### 10.3.5 代碼測試覆蓋率
- 有人認為它是非常有用的指標，應該要求達到一定比例的測試覆蓋度
- 有人認為它是有用的工具，但它
  - 只能告訴你「哪些程式碼缺乏測試案例覆蓋」
